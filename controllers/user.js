    ...
    if(email.length!=0){//user with same emailid has been found
		if(email.email_verified)//if his mail is verified
			...
			await axios.post('http://localhost:3000/users/send_verification',info).then((response)=>{
				return res.status(500).json({msg : 'otp has been sent to your mailid,please verify it for signing up' , token : response.data.token})
			})    
		}
    }
    else{ //solo user
    	...
    	await newuser.save()
    	return res.status(500).json('added succesfully,please verify your mail')
    } 	
}
